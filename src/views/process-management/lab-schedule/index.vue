  ...
  <div class="lab-schedule-container">
    <a-card title="实验室开放时间配置" class="page-card">
      ...
      <div class="search-section">
        <a-input v-model:value="searchForm.ruleName" placeholder="搜索规则名称" allow-clear @change="handleSearch" class="search-input">
          <template #prefix>
            <SearchOutlined />
          </template>
        </a-input>

        <a-select v-model:value="searchForm.status" placeholder="状态" allow-clear @change="handleSearch" class="search-input">
      ...
      <!-- 时间规则列表 -->
      <a-table :columns="columns" :data-source="filteredList" :pagination="pagination" :loading="loading" row-key="id" @change="handleTableChange" :scroll="{ x: 'max-content' }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'dateRange'"> {{ record.startDate }} ~ {{ record.endDate }} </template>
    ...
    <!-- 添加/编辑时间规则模态框 -->
    <a-modal v-model:open="modalVisible" :title="isEdit ? '编辑时间规则' : '添加时间规则'" width="900px" @ok="handleModalOk" @cancel="handleModalCancel">
      <a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
        ...
        <a-form-item label="适用实验室" name="laboratoryIds">
          <a-select v-model:value="formData.laboratoryIds" mode="multiple" placeholder="选择实验室（不选则需要后端控制 all 场景）">
        ...
        <a-form-item label="时间段设置" name="timeSlots">
          <div class="time-slots-config">
            <div style="margin-bottom: 24px">
              <span>可选时间段：</span>
              <a-space wrap>
                <a-tag v-for="slot in displayTimeSlotOptions" :key="slot.key" :color="selectedTimeSlots.includes(slot.key) ? 'blue' : ''" style="cursor: pointer" @click="toggleTimeSlot(slot.key)">
                  {{ slot.label }}
                </a-tag>
              </a-space>
            </div>
            <div class="time-slots-selected" v-if="selectedTimeSlots.length" style="margin-top: 8px">
              ...
              <a-space wrap>
                <a-tag v-for="key in selectedTimeSlots" :key="key" color="blue" closable @close.prevent="removeTimeSlot(key)">
                  {{ key }}
                </a-tag>
              </a-space>
            </div>
...
import { listAllLabScheduleService, getLabScheduleByIdService, createLabScheduleService, updateLabScheduleService, deleteLabScheduleService } from '@/api/labSchedule'
...
const formRules = {
  ...
  timeSlots: [
    {
      validator: () => {
        return selectedTimeSlots.value.length > 0 ? Promise.resolve() : Promise.reject(new Error('请选择时间段'))
      }
    }
  ]
}
...
const renderWeekdays = (val) => {
  ...
  return arr.map((d) => `周${map[d] || d}`).join('、')
}
...
const filteredList = computed(() => {
  ...
  if (searchForm.ruleName) {
    ...
    result = result.filter((item) => item.ruleName?.toLowerCase().includes(keyword))
  }
...
const loadList = async () => {
  loading.value = true
  const res = await listAllLabScheduleService(searchForm.ruleName || undefined, searchForm.ruleType || undefined, searchForm.status)
    ...
    Object.assign(formData, {
      ...
      timeSlotIds: (() => {
        ...
        try {
          ...
          return arr.map((it) => (it.id != null ? String(it.id) : null)).filter(Boolean)
        } catch (e) {
          return []
        }
      })(),
...
const handleModalOk = async () => {
  ...
  const res = isEdit.value ? await updateLabScheduleService(payload) : await createLabScheduleService(payload)
